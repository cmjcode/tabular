...
impl super::Tabular {
    ...
    pub fn render_tree(
        ...
        for (index, node) in nodes.iter_mut().enumerate() {
            ...
            if let Some((conn_id, db_name)) = backup_request {
                backup_requests.push((conn_id, db_name));
            }
        ...
        for (conn_id, db_name) in open_diagram_requests {
            self.open_database_diagram(conn_id, db_name);
        }
       ...
       || node.node_type == models::enums::NodeType::CustomFolder
        {
            ...
            ui.horizontal(|ui| {
                ...
                let icon = match node.node_type {
                    ...
                    models::enums::NodeType::CustomView => egui_icons::icons::ICON_VISIBILITY.codepoint,
                    ...
                    models::enums::NodeType::MetricsUserActiveFolder => egui_icons::icons::ICON_PERSON.codepoint,
                            ...
                            if supported {
                                ...
                                if ui.button("💾 Backup Database...").clicked() {
                                    let database_name = node
                                        ...
                                        .or_else(|| Some(node.name.clone()))
                                        .unwrap_or_default();
                                    backup_request = Some((conn_id, database_name));
                                    ui.close();
                                }
            ...
            if node.is_expanded {
                ...
                if is_history_date_folder {
                    for (child_index, child) in node.children.iter_mut().enumerate() {
                        ...
                        if let Some(v) = child_backup_request {
                            backup_request = Some(v);
                        }
                    ...
                    ui.indent(id, |ui| {
                        for (child_index, child) in node.children.iter_mut().enumerate() {
                            ...
                            if let Some(v) = child_backup_request2 {
                                backup_request = Some(v);
                            }
