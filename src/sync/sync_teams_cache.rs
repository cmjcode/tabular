...
use super::api_client::{RemoteSharedFolder, RemoteTeam, RemoteTeamMember};
...
pub async fn load_teams_from_cache(pool: &SqlitePool) -> Vec<RemoteTeam> {
    ...
    match res {
        Ok(rows) => rows
            .into_iter()
            .map(
                |(id, owner_id, name, description, created_at, updated_at)| RemoteTeam {
                    id,
                    owner_id,
                    name,
                    description,
...
pub async fn load_team_members_from_cache(
    pool: &SqlitePool,
) -> HashMap<String, Vec<RemoteTeamMember>> {
    ...
    if let Ok(rows) = res {
        for (team_id, user_id, email, display_name, avatar_url, username, phone, role, joined_at) in
            rows
        {
...
pub async fn load_shared_folders_from_cache(pool: &SqlitePool) -> Vec<RemoteSharedFolder> {
    ...
    match res {
        Ok(rows) => rows
            .into_iter()
            .map(
                |(id, team_id, resource_type, folder_path, created_at)| RemoteSharedFolder {
                    id,
                    team_id,
                    resource_type,
                    folder_path,
...
pub async fn delete_team_cache(pool: &SqlitePool, team_id: &str) {
    let _ = sqlx::query("DELETE FROM teams_cache WHERE id = ?")
        .bind(team_id)
        .execute(pool)
        .await;
    let _ = sqlx::query("DELETE FROM team_members_cache WHERE team_id = ?")
        .bind(team_id)
        .execute(pool)
        .await;
    let _ = sqlx::query("DELETE FROM team_shared_folders_cache WHERE team_id = ?")
        .bind(team_id)
        .execute(pool)
        .await;
}
...
pub async fn save_team_members_cache(
    pool: &SqlitePool,
    team_id: &str,
...
pub async fn save_shared_folders_cache(pool: &SqlitePool, folders: &[RemoteSharedFolder]) {
    let _ = sqlx::query("DELETE FROM team_shared_folders_cache")
        .execute(pool)
        .await;
    for f in folders {
        let _ = sqlx::query(
