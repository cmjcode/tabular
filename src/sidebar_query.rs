...
pub(crate) fn create_query_folder_in_parent(
    tabular: &mut window_egui::Tabular,
    folder_name: &str,
    parent_folder: &str,
) -> Result<(), String> {
...
pub(crate) fn move_query_to_folder(
    tabular: &mut window_egui::Tabular,
    query_file_path: &str,
    target_folder: &str,
) -> Result<(), String> {
    let source_path = std::path::Path::new(query_file_path);
    let file_name = source_path.file_name().ok_or("Invalid file path")?;
...
pub(crate) fn move_query_to_root(
    tabular: &mut window_egui::Tabular,
    query_file_path: &str,
) -> Result<(), String> {
    let source_path = std::path::Path::new(query_file_path);
                ...
                ui.horizontal(|ui| {
                    if ui.button("Create").clicked() {
                        ...
                        let result = if let Some(parent) = parent_folder {
                            sidebar_query::create_query_folder_in_parent(
                                tabular,
                                &folder_name,
                                &parent,
                            )
...
pub(crate) fn handle_query_remove_request_by_hash(
    tabular: &mut window_egui::Tabular,
    hash: i64,
) -> bool {
